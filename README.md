...
- 可在软件内选择三服统一 ZIP，版本和逐文件校验通过后自动导入差异包。
- 切换前检查游戏版本、差异包逐文件长度与 SHA-256、进程占用和缓存状态。
- 切换过程中保留事务备份，失败时自动回滚。
- 保存并恢复各服务器的 `version/revision` 元数据。
- 检测 `.zzzswitch` 存储目录、差异包和缓存异常，并可安全重建标准目录结构。
- 提供详细检查、备份历史、运行日志和恢复功能。
- 紧凑设置页集中管理中文/English、跟随 Windows/深色/浅色主题、缓存与备份位置、启动行为、窗口记忆和 7/30 天日志保留。
...
将 `ZZZSwitch-win-x64-v1.2.6.zip` 完整解压到普通文件夹，例如：
...
推荐启动软件、选择游戏目录后，点击主页“导入差异包”，选择完整的
`ZZZSwitch-Packages-3.1.0.zip`。软件会在临时目录解压，同时检查游戏版本、
三服目录结构和内置清单中的文件；全部通过后才替换同版本差异包目录。
导入失败不会用半份内容覆盖已有差异包，所选 ZIP 也不会被修改。
...
目录修复不会下载、生成或伪造差异文件，也不会修改当前游戏的 `Persistent\Blocks`。修复后仍需在主页导入对应版本的三服统一差异包，或手动解压到游戏目录上一级。
...
├─ ui-settings.json                # 语言、主题、启动行为、日志保留和窗口位置
└─ state.json                      # 最近一次成功状态
```

当前发布物是便携 ZIP，不注册卸载程序，也不把 ZZZSwitch 设置写入注册表。删除解压目录不会删除 `%LOCALAPPDATA%\ZZZSwitch`；同一 Windows 用户以后重新下载并运行新版时，仍会读取这里原有的设置、状态、日志和默认备份。若要完全重置，需要先确认没有待恢复事务或仍需保留的备份，再单独处理该应用数据目录。程序只读 Windows 主题注册表项以及 Windows 卸载列表，用于“跟随 Windows”主题和游戏目录自动检测。
...
1. 点击“自动检测”重新扫描。
2. 确认使用的是 ZZZSwitch 1.2.6 或与当前游戏版本配套的更新版本。
