...
1. 先按游戏版本、来源服和目标服查询本地自动差异包；存在 `Ready` 包时跳过 Sophon 分析和下载窗口，直接进入切换前 SHA-256 校验。
2. 首次缺包时，按当前游戏版本并行获取来源服与目标服 Sophon manifest，自动计算方向差异并分类。
3. 弹出下载窗口，显示文件数、最坏下载量、排除范围、逐文件/逐 chunk 实时网络进度、完整文件复用和分块断点命中状态；失败原因在独立的可换行区域显示。
4. 下载到 `%LOCALAPPDATA%\ZZZSwitch\OnlineDifferenceFiles\<version>\<target>\<manifest>\content`。下载器使用共享的最多四路分块并发；每个解压后 MD5 正确的 chunk 先写入同工作区的 `chunks` 断点缓存。取消或网络失败时只删除当前重建临时文件，重试会复用已完成文件和已验证分块。单个 chunk 采用单层最多六次自动重试；完整文件提交后清理其冗余分块缓存。
5. 每个 chunk、完整文件 MD5 和最终 SHA-256 全部通过后，原子生成动态 `TransitionManifest`；该工作区随即成为对应版本和目标服的本地自动差异包。
6. 把自动差异包交给原有 `SwitchPlanner` / `SwitchEngine`，继续执行 SHA-256、进程占用、磁盘空间、备份、预复制、事务日志、Blocks 缓存处理和失败回滚。
