# 设计说明

## 分层

- `ZZZSwitch`：WPF 界面、只读报告、切换预览、用户确认、进度和备份管理。
- `ZZZSwitch.Core`：配置、识别、路径安全、预检、备份、切换、回滚、状态与日志。
- `ZZZSwitch.Core.Tests`：无第三方测试框架的控制台测试器，只使用系统临时目录和模拟文件。

核心库不依赖 WPF，可以独立测试。文件操作和进程检测通过接口注入，以便故障注入测试不会触碰真实游戏目录。

主窗口采用渐进式 MVVM：`MainWindowViewModel` 管理路径、扫描摘要、报告、忙碌状态、交互可用性和界面 Command，XAML 不再直接绑定主窗口 `Click` 事件；`RelayCommand` / `AsyncRelayCommand` 统一 `CanExecute`、重复执行保护和异常转交。`InspectionPresentationBuilder` 将核心扫描结果转换为只读 UI 摘要和详细报告；`ServerSwitchWorkflow`、`CacheManagementWorkflow`、`BackupManagementWorkflow`、`PackageImportWorkflow`、`SettingsWorkflow` 分别编排切换、缓存、备份目录、差异包导入和全局设置，并通过 `MainWindowWorkflowContext` 回调窗口状态、进度和目录打开行为。核心 `SwitchPlanner`、`SwitchEngine`、恢复策略、跨进程互斥与回滚顺序保持不变。重复的三服入口由 `ServerSwitchCard` 统一图标、标题、说明与交互样式，客户端/版本/差异包/缓存区域由独立的 `InspectionSummaryCard` 承载；`ThemeManager` 以动态资源统一切换跟随 Windows、深色和浅色配色并同步窗口标题栏，`LocalizationManager` 动态切换主界面与设置页的中文/English 资源。设置页仅保留界面、存储、启动和日志四组常用选项；日志按选择的 7/30 天周期自动清理。

`MainWindowDialogCoordinator` 通过 `IMainWindowDialogs` 统一消息窗、游戏目录候选、系统文件夹选择、切换确认、缓存管理和备份窗口的 Owner/取消语义，也让工作流测试可替换交互边界；`StartupWorkflow` 仅编排既有的未完成事务恢复、状态警告传递和备份轮换。实际恢复仍由核心 `PendingTransactionRecoveryService` 执行，最后备份保护路径仍来自重新加载的 `state.json`，轮换失败继续采用不阻断启动的 best-effort 规则。

## 维护约束

- 三服入口共用切换工作流；保留兼容路径时说明它服务于哪种已有数据，避免新旧执行流程长期并存。
- 新增服务、接口或配置必须解决当前需求；优先复用现有边界。提取方法应减少理解所需的上下文，不以文件行数作为拆分目标。
- 事务顺序、提交边界和恢复责任集中可查；纯重构与行为修改分开验证。
- 回归测试优先检查实际文件、恢复结果和用户输出，避免仅调用私有策略或格式化方法来证明行为。
- 每轮改动同步相关设计和测试说明，交接记录区分已提交版本、工作区改动和临时构建。

## 识别策略

1. 读取 `state.json` 作为提示，但不以其覆盖物理文件结果。
2. 对每个启用 profile 的少量稳定关键文件检查存在性与精确大小。`version` / `revision` 热更新元数据不作为固定大小签名，避免有效缓存快照造成误判。
3. 单个 profile 全部匹配时识别为该服；多个 profile 同时完整匹配时识别为混合状态。
4. 没有完整匹配但多个 profile 均出现部分证据时识别为混合状态，否则为未知状态。
5. 配置模型保留可选 SHA-256 字段，可针对少量关键文件启用，不要求全目录哈希。

当前关键文件大小来自 2026-07-30 对版本 3.1.0 差异包的只读盘点。

## 事务边界

切换顺序：预检 → 筛选实际需要变化的文件 → 写入 `Staging` 事务记录 → 在游戏同卷的 `.zzzswitch\staging\<operation>` 暂存差异文件并按 Manifest 校验源和副本 → 保存来源服元数据快照 → 备份实际受影响文件 → 事务推进至 `Prepared` → 交换 Blocks（或准备目标服首次初始化目录）→ 替换文件及修改 INI → 删除 → 恢复有效目标服元数据快照 → 数量/长度/状态校验 → 保存备份结果及审计日志 → 最后提交 `state.json`。Blocks 缓存位于另一磁盘时，“移动”会展开为目标卷临时复制、校验、同卷原子落位、最后删除源目录。

`Prepared` 之前失败时尚未修改游戏文件，只清理本次暂存；开始修改后、提交状态之前失败时，先恢复 Blocks，再恢复普通文件。进度回调异常不会影响事务。状态写入是最后一个可能影响提交语义的步骤；后续清理失败不会将已提交切换改判为失败。

文件系统操作无法保证断电级原子性。下次启动根据事务记录和已提交状态恢复：`Staging` 阶段只清理明确属于该操作的暂存；进入修改阶段后联合恢复 Blocks 和普通文件；已提交的操作只清理遗留记录。暂存被占用时保留事务供重试，未完成备份及无日志的历史暂存保留供检查。手动恢复已有备份仍受当前 Blocks 状态等恢复策略约束。

每次事务的 `backup.json` 和受影响文件都保存在 `.zzzswitch\app-data\backup-records\<操作>` 下，文件正文位于该操作自己的 `files` 子目录。备份记录不保存正文 SHA-256，也不建立内容寻址数据库；复制过程本身仍验证源和目标一致。曾由测试版创建的 `.zzzswitch\app-data\backup-content\sha256` 只用于兼容恢复已有记录，不再创建或自动回收。用户可从主界面迁移整个备份目录；“恢复上次状态”属于备份历史窗口，并继续使用 AppData `state.json` 中的精确操作关联。

## 按服缓存快照

快照只枚举以下两个目录的一级普通文件：

- `ZenlessZoneZero_Data\Persistent`
- `ZenlessZoneZero_Data\StreamingAssets`

文件名必须包含 `version` 或 `revision`（不区分大小写）。不递归，不复制 `Blocks` 等目录。每个文件只记录相对路径和长度；快照按游戏安装身份、版本和资源服隔离，路径只由实际 `snapshot.json` 所在目录确定，不信任记录中的历史绝对路径。读取目标快照时验证 profile、游戏版本、原游戏路径、路径边界和文件长度；每组保留最近两份有效快照，损坏记录保留供人工检查并自动跳过。

2026-07-22 的真实国际服只读盘点会纳入 20 个一级文件，共 4,471,666 字节。

## 资源服 Blocks 缓存

`ZenlessZoneZero_Data\Persistent\Blocks` 不再纳入小型元数据快照，而由 `HotUpdateCacheService` 单独管理：

- 初始化只登记当前活动 Blocks 的文件数量和总大小，不移动游戏文件。
- 默认位置使用统一的 `.zzzswitch` 根目录；`blocks-cache` 按规范化游戏路径的身份哈希隔离每份安装并保存大体积 Blocks 仓库，`packages` 保存按版本分组的只读离线输入，其他恢复数据和下载工作区位于 `app-data`。用户可以逐安装迁移 Blocks 缓存根目录；同卷使用目录移动，跨卷使用可恢复的校验复制。
- 切走时重新生成来源服清单并移动整个 Blocks 目录；切入时恢复目标服目录。
- B服是国服资源上的登录组件叠加层，统一映射到 `cn_official`，不创建第三套 Blocks 或元数据快照。
- 国服与 B服互切只应用内嵌 B服覆盖层；国际服与 B服互切由 Sophon 在线国际服/国服基础差异和内嵌覆盖层组合成一次事务，不再要求 `.zzzswitch\packages\<版本>\global` 或 `cn_official` 固定全量目录。已有固定目录作为旧版离线包保留，不会在未确认时自动删除。
- 原资源包中的“国服/国际服热更新存储链接”只作为首次初始化种子，只读复制，不回写。
- `Persistent\Video` 完全不在工具管理范围内：不保存、不交换、不校验、不复制、不删除。
- 首次目标服缓存不存在时进入一次性下载模式；切走当前服前自动保存其 Blocks，目标服进入游戏完成下载后无需手动初始化，下次切走时自动保存。
- 每次交换写入应用私有事务日志。普通文件替换失败时，Blocks 与普通文件共同回滚。
- 事务日志在首次移动前已经落盘；恢复逻辑会区分“尚未移动”“跨卷复制形成安全副本”“目标已激活”和“回滚已完成”状态，避免删除唯一缓存。
- “清理旧版本缓存”统一处理当前安装的旧版 Blocks、安装范围 Manifest、旧布局 Manifest 和元数据快照；无法确认归属或内容损坏的目录不会自动删除。
